...
export function ApprovalsPage() {
  ...
  const act = (item: WorkspaceApproval, status: "Approved" | "Rejected") => {
    ...
    recordAudit({ actor: "Current user", actorType: "Human user", action: `${status} AI action`, entity: item.title, before: item.status, after: status, status: "Completed", approval: `Approval #${item.id}`, source: "Approval Center" });
  };
