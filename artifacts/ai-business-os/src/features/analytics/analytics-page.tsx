...
export function AnalyticsPage() {
  ...
  const learn = useMutation({ mutationFn: () => marketingApi.performance.learn(activeBusinessId, period.start, period.end), onSuccess: (result) => { setNotice(result.created ? "A cautious, provenance-linked marketing conclusion was saved to AI Memory." : "More data across at least two channels is required before AI Memory learns from this period."); setActionError(""); }, onError: (reason) => setActionError(humanizeApiError(reason, "Marketing learning could not be completed.")) });
  ...
  return <>
    ...
    {query.isLoading || !analytics ? <Card><div className="empty"><p>Calculating operational analytics…</p></div></Card> : <>
      ...
      <PageHeader eyebrow="Marketing performance" title="Campaign intelligence" subtitle="Manual/imported records with server-derived CTR, CPC, CPL, CPA, and ROAS." action={<Button disabled={learn.isPending} onClick={() => learn.mutate()}><Sparkles /> {learn.isPending ? "Evaluating…" : "Learn from sufficient data"}</Button>} />
      <div className="grid analytics-kpi-grid"><Metric title="Marketing spend" value={money(marketing.data.spend)} icon={<Target />} tone="orange" /><Metric title="Attributed revenue" value={money(marketing.data.revenue)} icon={<TrendingUp />} tone="green" /><Metric title="ROAS" value={`${Number(marketing.data.roas).toFixed(2)}x`} icon={<BarChart3 />} tone="brown" /><Metric title="Conversions" value={String(marketing.data.conversions)} icon={<Sparkles />} tone="rose" /><Metric title="Impressions" value={marketing.data.impressions.toLocaleString()} icon={<Users />} tone="green" /><Metric title="CTR" value={`${Number(marketing.data.ctr).toFixed(2)}%`} icon={<BarChart3 />} tone="orange" /><Metric title="CPC" value={money(marketing.data.cpc)} icon={<Target />} tone="brown" /><Metric title="Leads" value={String(marketing.data.leads)} icon={<Users />} tone="rose" /></div>
