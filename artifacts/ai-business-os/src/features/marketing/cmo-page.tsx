...
export function CmoPage() {
  ...
  return <>
    ...
    {initialLoading ? <Card><div className="empty"><RefreshCw className="spin" /><p>Assembling the marketing workspace…</p></div></Card> : <>
      ...
      <div className="grid split-grid"><Card><SectionTitle title="Content preview" action={primary && <Badge tone={primary.status === "approved" || primary.status === "ready_to_publish" ? "success" : "warning"}>{primary.status.replaceAll("_", " ")}</Badge>} />{content.isError ? <div className="empty"><AlertCircle /><h3>Content drafts could not load</h3><p>{humanizeApiError(content.error, "Retry content. Publishing connections are not required for drafting.")}</p><Button onClick={() => void content.refetch()}>Retry content</Button></div> : content.isLoading ? <div className="empty"><RefreshCw className="spin" /><p>Loading content…</p></div> : primary ? <><div className="card" style={{ padding: 18, background: "#f9f6ef", borderColor: "#eee3d1" }}><div className="eyebrow">{primary.channel} · {primary.content_type.replaceAll("_", " ")} · version {primary.version}</div><h2 style={{ lineHeight: 1.5 }}>{primary.body}</h2>{primary.cta && <p className="subtle" style={{ marginTop: 12 }}>CTA · {primary.cta}</p>}</div><div className="toolbar" style={{ marginTop: 13 }}><Button variant="secondary" className="btn-sm" disabled={regenerate.isPending} onClick={() => regenerate.mutate(primary)} data-testid="button-regenerate-content"><RefreshCw /> Regenerate version</Button>{["draft", "review"].includes(primary.status) && <Button variant="soft" className="btn-sm" disabled={approve.isPending} onClick={() => approve.mutate(primary)} data-testid="button-approve-content"><Check /> Approve</Button>}{primary.status === "approved" && <Button variant="primary" className="btn-sm" onClick={() => setSchedule(primary)} data-testid="button-schedule-content"><Calendar /> Schedule</Button>}</div></> : <div className="empty"><Wand2 /><h3>No content draft</h3><p>Generate grounded copy now; a social provider is needed only for publishing.</p><Button variant="soft" onClick={() => setShowContentGenerator(true)}>Create AI content plan</Button></div>}</Card><Card><SectionTitle title="Content calendar" action={<Badge>{calendar.data?.length ?? 0} upcoming</Badge>} />{calendar.isError ? <div className="empty"><AlertCircle /><h3>Calendar could not load</h3><p>{humanizeApiError(calendar.error, "Retry the internal calendar.")}</p><Button onClick={() => void calendar.refetch()}>Retry calendar</Button></div> : calendar.isLoading ? <div className="empty"><RefreshCw className="spin" /><p>Loading calendar…</p></div> : <>{calendar.data?.slice(0, 8).map((item) => { const contentItem = content.data?.items.find((value) => value.id === item.content_id); return <div className="list-row" key={item.id}><div style={{ width: 86, color: "#938c83", fontSize: 10 }}>{new Date(item.scheduled_for).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</div><div className="row-main"><div className="row-title">{contentItem?.title || `${item.channel} content`}</div><div className="row-copy">{new Date(item.scheduled_for).toLocaleTimeString()} · {item.timezone}</div></div><Badge tone="success">{item.status.replaceAll("_", " ")}</Badge></div>; })}{!calendar.data?.length && <div className="empty"><Calendar /><h3>No content scheduled</h3><p>Approved content can be added to the internal calendar without a publishing provider.</p></div>}</>}</Card></div>
    </>}
    {showContentGenerator && <Modal title="Generate content" description="The AI CMO uses trusted brand, catalog, Business Brain, and memory context." onClose={() => setShowContentGenerator(false)}><form onSubmit={(event) => generateContent.mutate(event)}><div className="form-grid"><div className="field full"><label>What should we create?</label><textarea name="prompt" required maxLength={4000} placeholder="Describe the goal, offer, and intended audience. Product facts must already exist in Business Brain or catalog." /></div><div className="field"><label>Title (optional)</label><input name="title" maxLength={180} /></div><div className="field"><label>Campaign</label><select name="campaign_id"><option value="">Standalone content</option>{campaigns.data?.items.map((campaign) => <option value={campaign.id} key={campaign.id}>{campaign.name}</option>)}</select></div><div className="field"><label>Channel</label><select name="channel" defaultValue="instagram">{channels.map((channel) => <option key={channel} value={channel}>{channel.replaceAll("_", " ")}</option>)}</select></div><div className="field"><label>Content type</label><select name="content_type" defaultValue="social_post">{contentTypes.map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></div><div className="field"><label>Language</label><input name="language" defaultValue={activeBusiness?.locale || "en"} /></div></div>{error && <p className="form-error">{error}</p>}<div className="modal-foot"><Button type="button" onClick={() => setShowContentGenerator(false)}>Cancel</Button><Button variant="primary" type="submit" disabled={generateContent.isPending}><Wand2 /> {generateContent.isPending ? "Generating…" : "Generate internal draft"}</Button></div></form></Modal>}
