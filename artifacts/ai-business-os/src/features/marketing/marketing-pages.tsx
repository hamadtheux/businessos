...
export function SocialManagementPage() {
  ...
  const createBrief = useMutation({
    ...
    onSuccess: () => {
      setBriefing(false);
      setNotice(
        "Creative brief was saved. Image generation remains provider-disabled.",
      );
      setError("");
      void invalidate();
    },
    onError: (reason) =>
      setError(
        humanizeApiError(reason, "Creative brief could not be prepared."),
      ),
  });
    ...
    <>
      ...
      <div className="social-channel-strip">
        {["instagram", "facebook", "linkedin", "tiktok"].map((platform) => {
          ...
          const registered = ["instagram", "facebook"].includes(platform);
          return (
            <Card key={platform} className="social-channel">
              ...
              <div className="row-main">
                ...
                <span>
                  {registered
                    ? "Authenticated external writes are provider-disabled"
                    : "No connector is registered; planning only"}
                </span>
              </div>
              <Badge tone="neutral">
                {registered ? "Provider disabled" : "Planning only"}
              </Badge>
            </Card>
          );
        })}
      ...
      {selected && (
        ...
        >
          ...
          <SectionTitle
            ...
            action={
              <Button className="btn-sm" onClick={() => setBriefing(true)}>
                <Sparkles /> Create creative brief
              </Button>
            }
          />
          {assets.isLoading && (
            <p className="subtle">Loading creative assets…</p>
          )}
          ...
          <div className="ai-banner">
            <AlertCircle /> Creative output remains a textual brief until a
            generation provider is configured. Publishing requires AIAction
            policy, approval, and a real authenticated write provider; current
      ...
      {briefing && selected && (
        <Modal
          ...
          description="Prepare durable visual direction now. Binary generation stays disabled until a provider is explicitly configured."
          ...
          <form onSubmit={(event) => createBrief.mutate(event)}>
            <div className="form-grid">
              <div className="field">
                ...
                <select name="asset_type" defaultValue="social_square">
              ...
              <div className="field">
                ...
                <input name="aspect_ratio" maxLength={16} defaultValue="1:1" />
              </div>
              <div className="field full">
