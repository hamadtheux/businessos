...
export function AgentDetailPage({ params }: { params: { agentId: string } }) {
  ...
  const save = (autonomy: typeof agent.autonomy, escalation: string) => {
    ...
    recordAudit({ actor: "Current user", actorType: "Human user", action: "Updated agent configuration", entity: agent.name, after: `${autonomy} autonomy`, status: "Completed", source: "AI Agents" });
