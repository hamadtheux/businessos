...
export function AgentsOverviewPage() {
  ...
  return <>
    <PageHeader eyebrow="Virtual team" title="AI employees" subtitle="Configure real role capabilities, bounded autonomy, and inspect durable activity." action={<Button onClick={() => setActiveOnly((value) => !value)}><Settings2 /> {activeOnly ? "Show all" : "Show active"}</Button>} />
    {error && <ErrorState message={error} retry={() => void load()} />}
    {loading && !agents.length ? <div className="empty"><RefreshCw className="spin" /><h3>Loading AI workforce</h3></div> : <div className="grid agent-grid">{agents.filter((agent) => !activeOnly || agent.enabled).map((agent) => { const Icon = icons[agent.role]; return <Card className={`agent-card agent-${agent.role} ${agent.enabled ? "active" : ""}`} key={agent.role}><div className="agent-head"><div className="agent-identity"><div className="agent-icon"><Icon /></div><div><h3>{agent.display_name}</h3><div className="agent-role">{agent.role_description}</div></div></div><button className={`switch ${agent.enabled ? "on" : ""}`} onClick={() => void toggle(agent)} aria-label={`Toggle ${agent.display_name}`}><i /></button></div><div className="stat-row"><span>Autonomy</span><Badge tone={agent.autonomy_mode === "autonomous" ? "success" : agent.autonomy_mode === "supervised" ? "warning" : "neutral"}>{agent.autonomy_mode}</Badge></div><div className="stat-row"><span>Executions</span><strong>{agent.metrics.execution_count}</strong></div><div className="stat-row"><span>Pending approvals</span><strong>{agent.metrics.pending_approval_count}</strong></div><div className="stat-row"><span>Last activity</span><strong>{agent.last_activity_at ? new Date(agent.last_activity_at).toLocaleString() : "No activity"}</strong></div><div className="agent-actions"><Link href={`/agents/${agent.role}`} className="btn btn-sm btn-secondary"><Settings2 /> Configure</Link><Link href={`/agents/${agent.role}/activity`} className="btn btn-sm btn-soft"><Activity /> Activity</Link></div></Card>; })}</div>}
    <SectionTitle title="Agent activity" action={<Link href="/agents/activity" className="btn btn-sm btn-secondary">Full activity</Link>} />
    <Card><div className="list">{activity.map((item) => <Link href={`/agents/${item.role}/activity`} className="list-row" key={item.id}><div className="activity-dot" /><div className="row-main"><div className="row-title">{item.role.replaceAll("_", " ")} <span className="row-copy inline-copy">· {item.task_summary}</span></div><div className="row-copy">{new Date(item.created_at).toLocaleString()} · {item.trigger}</div></div><Badge tone={tone(item.status)}>{item.status.replaceAll("_", " ")}</Badge></Link>)}{!loading && !activity.length && <div className="empty compact-empty"><Activity /><h3>No agent activity</h3><p>Durable executions will appear here.</p></div>}</div></Card>
  </>;
}
...
export function AgentDetailPage({ params }: { params: { agentId: string } }) {
  ...
  return <>
    ...
    <div className="grid split-grid"><Card><SectionTitle title="Configuration" action={<Settings2 />} /><div className="field"><label>Display name</label><input value={displayName} maxLength={100} onChange={(event) => setDisplayName(event.target.value)} /></div><div className="field"><label>Autonomy</label><select value={autonomy} onChange={(event) => setAutonomy(event.target.value as AutonomyMode)}><option value="manual">Manual</option><option value="supervised">Supervised</option><option value="autonomous">Autonomous</option></select></div><div className={`autonomy-explainer ${autonomy}`}><Gauge /><div><strong>{autonomy}</strong><p>{autonomyCopy[autonomy]}</p></div></div><div className="field" style={{ marginTop: 18 }}><label>Custom preference instructions</label><textarea value={instructions} maxLength={2000} onChange={(event) => setInstructions(event.target.value)} placeholder="Tone and business preferences only. Do not enter secrets." /><span className="subtle">{instructions.length}/2000 · Cannot override system safety, policy, or tenant boundaries.</span></div><div className="toolbar"><Button variant="green" onClick={() => void save()} disabled={saving || !displayName.trim()}><Save /> {saving ? "Saving…" : "Save configuration"}</Button><Button onClick={() => void reset()} disabled={saving}><RefreshCw /> Reset defaults</Button></div></Card><Card><SectionTitle title="Server-approved capabilities" action={<ShieldCheck />} /><div className="permission-list">{agent.default_capabilities.map((capability) => <label className="permission" key={capability}>{capability.replaceAll("_", " ")}<input type="checkbox" checked={selected.includes(capability)} onChange={() => setSelected((current) => current.includes(capability) ? current.filter((item) => item !== capability) : [...current, capability])} /></label>)}</div><div className="risk-note"><ShieldCheck /><div><strong>Human oversight remains active</strong><p>Capability selection can only narrow the role’s server allowlist. It never weakens mandatory approval, spend, communication, or destructive-action policy.</p></div></div></Card></div>
  </>;
}
