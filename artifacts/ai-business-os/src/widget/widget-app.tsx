  ...
  return <main className={`widget-shell theme-${config.theme}`} style={style} aria-label={`Chat with ${config.business_name}`}>
    ...
    {panel === "chat" && <>
      <section ref={messageListRef} className="message-list" aria-live="polite" aria-label="Conversation messages">
        {messages.map((message) => <article className={`message ${message.sender}`} key={message.id}><p>{message.text}</p>{message.products?.length ? <div className="product-list">{message.products.map((product) => <div className="product-card" key={product.reference}><span>{product.item_type}</span><strong>{product.name}</strong>{product.description && <p>{product.description}</p>}<b>{product.price == null ? "Ask for pricing" : formatPrice(product.price, product.currency)}</b></div>)}</div> : null}</article>)}
...
function OrderForm({ api, onBack, onComplete }: { api: PublicWidgetApi; onBack: () => void; onComplete: (message: string) => void }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); setError(""); const form = new FormData(event.currentTarget); try { const value = await api.orderStatus({ order_reference: String(form.get("reference") || ""), email: optional(form.get("email")), phone: optional(form.get("phone")) }); onComplete(`Order ${value.order_reference} is ${value.status.replaceAll("_", " ")}.`); } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(false); } };
