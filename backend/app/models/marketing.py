...
class CreativeAsset(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("asset_type IN ('social_square','story_reel','landscape_ad','display_banner','creative_brief','other','video_source','video_vertical','video_landscape','video_square')", name="valid_asset_type"),
        CheckConstraint("media_type IN ('image','video')", name="valid_media_type"),
        CheckConstraint("(media_type = 'image' AND asset_type IN ('social_square','story_reel','landscape_ad','display_banner','creative_brief','other')) OR (media_type = 'video' AND asset_type IN ('video_source','video_vertical','video_landscape','video_square'))", name="consistent_media_asset_type"),
        ...
        CheckConstraint("(media_type = 'image' AND duration_seconds IS NULL) OR (media_type = 'video' AND ((duration_seconds IS NOT NULL AND duration_seconds BETWEEN 1 AND 3600) OR (duration_seconds IS NULL AND source_type = 'import' AND asset_type = 'video_source' AND generation_status IN ('processing','failed'))))", name="consistent_media_duration"),
        CheckConstraint("asset_type <> 'video_source' OR (media_type = 'video' AND source_type = 'import' AND generation_status IN ('processing','failed'))", name="consistent_video_source_state"),
        CheckConstraint("generation_status <> 'processing' OR (media_type = 'video' AND source_type = 'import' AND asset_type = 'video_source' AND storage_reference IS NOT NULL AND width IS NULL AND height IS NULL AND aspect_ratio IS NULL AND duration_seconds IS NULL AND provider_key IS NULL AND provider_job_reference IS NULL)", name="consistent_processing_video_state"),
        CheckConstraint("NOT (media_type = 'video' AND generation_status = 'ready' AND creative_metadata ? 'video_preparation') OR (COALESCE(creative_metadata #>> '{video_preparation,status}', '') = 'ready' AND asset_type IN ('video_vertical','video_landscape','video_square') AND storage_reference IS NOT NULL AND width BETWEEN 1 AND 20000 AND height BETWEEN 1 AND 20000 AND char_length(btrim(aspect_ratio)) BETWEEN 3 AND 16 AND duration_seconds BETWEEN 1 AND 3600)", name="consistent_ready_video_metadata"),
